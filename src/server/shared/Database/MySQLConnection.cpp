...
bool MySQLConnection::Open()
{
    ...
    if (m_Mysql)
    {
        if (!m_reconnecting)
        {
            ...
            if (mysql_get_server_version(m_Mysql) != mysql_get_client_version())
                sLog->outSQLDriver("[INFO] MySQL Client / Server-Version; können Konflikte verursachen!.");
        }

        sLog->outDetail("Verbunden mit der MySQL-Datenbank %s", m_connectionInfo.host.c_str());
    ...
    {
        sLog->outError("Keine Verbindung zur MySQL-Datenbank %s: %s\n", m_connectionInfo.host.c_str(), mysql_error(mysqlInit));
...
bool MySQLConnection::Execute(const char* sql)
{
...

    {
        ...
        if (mysql_query(m_Mysql, sql))
        {
            ...
            sLog->outSQLDriver("Fehler: [%u] %s", lErrno, mysql_error(m_Mysql));
    ...
    for (itr = queries.begin(); itr != queries.end(); ++itr)
    {
        ...
        switch (itr->type)
        {
            case SQL_ELEMENT_PREPARED:
            {
                ...
                if (!Execute(stmt))
                {
                    sLog->outSQLDriver("[Warning] Transaktion abgebrochen. %u Anfrage nicht ausgeführt.", (uint32)queries.size());
                    RollbackTransaction();
                    return false;
                }
            }
            ...
            case SQL_ELEMENT_RAW:
            {
                ...
                if (!Execute(sql))
                {
                    sLog->outSQLDriver("[Warning] Transaktion abgebrochen. %u Abfrage nicht ausgeführt.", (uint32)queries.size());
                    RollbackTransaction();
                    return false;
                }
            }
