...
class DatabaseWorkerPool
{
    public:
        ...
        m_queue(new ACE_Activation_Queue(new ACE_Message_Queue<ACE_MT_SYNCH>))
        {
            ...
            WPFatal (mysql_thread_safe(), "MySQL-Bibliothek ist nicht Thread-sicher.");
        }

        ~DatabaseWorkerPool()
        {
            sLog->outSQLDriver("~DatabaseWorkerPool für '%s'.", m_connectionInfo.database.c_str());
        }
        ...
        bool Open(const std::string& infoString, uint8 async_threads, uint8 synch_threads)
        {
            ...
            sLog->outSQLDriver("Öffne Databasepool '%s'. Async Themen: %u, Synch Themen: %u", m_connectionInfo.database.c_str(), async_threads, synch_threads);
            ...
            sLog->outSQLDriver("Databasepool erfolgreich geöffnet. %u Verbindungen aktiv.", (m_connectionCount[IDX_SYNCH] + m_connectionCount[IDX_ASYNC]));
            return res;
        }

        void Close()
        {
            sLog->outSQLDriver("Schließe Databasepool '%s'.", m_connectionInfo.database.c_str());
            ...
            sLog->outSQLDriver("Asynchrone Verbindungen auf Databasepool '%s' beendet. Ausgehende Synchron-Verbindungen.", m_connectionInfo.database.c_str());
            ...
            sLog->outSQLDriver("Alle Verbindungen auf Databasepool %s geschlossen.", m_connectionInfo.database.c_str());
        }
        ...
        */
        ...
        void CommitTransaction(SQLTransaction transaction)
        {
            if (sLog->GetSQLDriverQueryLogging())
            {
                ...
                {
                    case 0:
                        sLog->outSQLDriver("Transaction enthält 0 Abfragen.");
                        return;
                    case 1:
                        sLog->outSQLDriver("Warning: Transaction hält nur 1 Abfrage, Transaktionskontext Code in Erwägung ziehen.");
                        break;
                    default:
                        break;
                }
