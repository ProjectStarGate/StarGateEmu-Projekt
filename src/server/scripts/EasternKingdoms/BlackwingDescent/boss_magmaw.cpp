...
public:
	...
	{
		boss_magmawAI(Creature* pCreature) : BossAI(pCreature, DATA_MAGMAW)
        {    
        }
        ...
        void Reset()
        {
            ...
            if (instance && (instance->GetData(DATA_MAGMAW_EVENT) != DONE && !check_in))
                instance->SetData(DATA_MAGMAW_EVENT, NOT_STARTED);
        ...
        void JustDied(Unit* /*Kill*/)
        {
			_JustDied();
            if (instance)
                instance->SetData(DATA_MAGMAW_EVENT, DONE);
        }
...
//            DoScriptText(RAND(SAY_KILL_1, SAY_KILL_2), me);
        ...
        void EnterCombat(Unit* /*Ent*/)
        {
			...
			if (instance)
                instance->SetData(DATA_MAGMAW_EVENT, IN_PROGRESS);
