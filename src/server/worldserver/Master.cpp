...
public:
    ...
    void run(void)
    {
        if (!_delaytime)
            return;
        sLog->outString("Starte Anti-Freeze-System (%u )...", _delaytime/1000);
        m_loops = 0;
        w_loops = 0;
        m_lastchange = 0;
        w_lastchange = 0;
        while (!World::IsStopped())
        {
            ...
            {
                sLog->outError("Welt hängt!!, Restart-System aktiv!");
                ASSERT(false);
            }
        }
        sLog->outString("Anti-Freeze-System kann nicht gestartet werden!.");
    }
};
...
int Master::Run()
{
    ...
    #ifdef _WIN32
    {
            ...
            if (GetProcessAffinityMask(hProcess, &appAff, &sysAff))
            {
                ...
                else
                {
                    if (SetProcessAffinityMask(hProcess, curAff))
                        sLog->outString("Prozessoren (bitmask, hex): %x", curAff);
                    else
                        sLog->outError("Es kann nicht festgelegt werden um welchen Prozessor es sich handelt! (hex): %x", curAff);
                }
            }
            sLog->outString("");
        }
        ...
        if (Prio)
        {
            if (SetPriorityClass(hProcess, HIGH_PRIORITY_CLASS))
                sLog->outString("[StarGate-Server] Prozess-Priorität festgelegt auf HOCH");
            else
                sLog->outError("Kann WorldServer Prozess-Priorität nicht setzten.");
            sLog->outString("");
        }
    }
    #endif
    ...
    sLog->outString("%s (StarGate-Server Online!)", _FULLVERSION);
...
bool Master::_StartDB()
{
    ...
    sLog->outString("Realm läuft auf Realm ID: %d", realmID);
    ...
    sLog->outString("DB-World: %s", sWorld->GetDBVersion());
    sLog->outString("Kreatur AI: %s", sWorld->GetCreatureAIVersion());
    return true;
}
