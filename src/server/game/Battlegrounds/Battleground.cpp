...
namespace Trinity
{
    class BattlegroundChatBuilder
    {
        public:
            ...
            {
                char const* text = sObjectMgr->GetStarGateString(_textId, loc_idx);
                if (_args)
                {
    ...
    class Battleground2ChatBuilder
    {
        public:
            ...
            {
                char const* text = sObjectMgr->GetStarGateString(_textId, loc_idx);
                char const* arg1str = _arg1 ? sObjectMgr->GetStarGateString(_arg1, loc_idx) : "";
                char const* arg2str = _arg2 ? sObjectMgr->GetStarGateString(_arg2, loc_idx) : "";
...
{
    const char *format = sObjectMgr->GetStarGateStringForDBCLocale(entry);
...
// To be removed
const char* Battleground::GetStarGateString(int32 entry)
{
    // FIXME: now we have different DBC locales and need localized message for each target client
    return sObjectMgr->GetStarGateStringForDBCLocale(entry);
}
