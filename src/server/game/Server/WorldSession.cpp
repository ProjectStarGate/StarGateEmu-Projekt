...
{
    char const* format = GetStarGateString(string_id);
    if (format)
    {
        va_list ap;
        char szStr [1024];
...
const char * WorldSession::GetStarGateString(int32 entry) const
{
    return sObjectMgr->GetStarGateString(entry, GetSessionDbLocaleIndex());
}
