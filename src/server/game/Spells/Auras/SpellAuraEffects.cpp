...
pAuraEffectHandler AuraEffectHandler[TOTAL_AURAS]=
{
    ...
    &AuraEffect::HandleNoImmediateEffect,                         //286 SPELL_AURA_ABILITY_PERIODIC_CRIT patch 4.0.1 dots crit
            ...
            switch(GetSpellProto()->SpellFamilyName)
            {
                ...
                case SPELLFAMILY_PALADIN:
                    ...
                    if (m_spellProto->SpellFamilyFlags[1] & 0x80000)
                    {
                        ...
                        DoneActualBenefit += caster->GetTotalAttackPowerValue(BASE_ATTACK) * bonus;
    ...
    switch(GetAuraType())
    {
      ...
      switch (GetId()) 
        ...
        case SPELL_AURA_OBS_MOD_POWER:
        {
            ...
            if(GetBase()->GetId() == 6117)
            {
            if(target->HasAura(56383)) //Glyph of Mage Armor
            amount += amount * 0.2; //+20% bonus
            }
...
void AuraEffect::TriggerSpell(Unit *target, Unit *caster) const
{
    ...
    else
    {
        ...
        if (!c || (c && !sScriptMgr->OnDummyEffect(caster, GetId(), SpellEffIndex(GetEffIndex()), triggerTarget->ToCreature())) ||
            ...
            sLog->outDebug(LOG_FILTER_SPELLS_AURAS, "AuraEffect::TriggerSpell: Spell %u has value 0 in EffectTriggered[%d] and is therefor not handled. Define as custom case?", GetId(), GetEffIndex());
    }
}
