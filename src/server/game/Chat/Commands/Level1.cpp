...
bool ChatHandler::HandleAnnounceCommand(const char* args)
{
    ...
    char buff[2048];
    sprintf(buff, GetStarGateString(LANG_SYSTEMMESSAGE), args);
...
bool ChatHandler::HandleNotifyCommand(const char* args)
{
    ...
    std::string str = GetStarGateString(LANG_GLOBAL_NOTIFY);
    str += args;
...
bool ChatHandler::HandleGMNotifyCommand(const char* args)
{
    ...
    std::string str = GetStarGateString(LANG_GM_NOTIFY);
    str += args;
...
bool ChatHandler::HandleGMVisibleCommand(const char* args)
{
    if (!*args)
    {
        PSendSysMessage(LANG_YOU_ARE, m_session->GetPlayer()->isGMVisible() ?  GetStarGateString(LANG_VISIBLE) : GetStarGateString(LANG_INVISIBLE));
        return true;
    }
...
bool ChatHandler::HandleSummonCommand(const char* args)
{
    ...
    else
    {
        ...
        PSendSysMessage(LANG_SUMMONING, nameLink.c_str(), GetStarGateString(LANG_OFFLINE));
...
bool ChatHandler::HandleModifyEnergyCommand(const char* args)
{
    ...
    sLog->outDetail(GetStarGateString(LANG_CURRENT_ENERGY), chr->GetMaxPower(POWER_ENERGY));
...
bool ChatHandler::HandleModifyMoneyCommand(const char* args)
{
    ...
    if (addmoney < 0)
    {
        ...
        sLog->outDetail(GetStarGateString(LANG_CURRENT_MONEY), moneyuser, addmoney, newmoney);
        if (newmoney <= 0)
        {
    ...
    sLog->outDetail(GetStarGateString(LANG_NEW_MONEY), moneyuser, addmoney, chr->GetMoney());
...
bool ChatHandler::HandleWhispersCommand(const char* args)
{
    if (!*args)
    {
        PSendSysMessage(LANG_COMMAND_WHISPERACCEPTING, m_session->GetPlayer()->isAcceptWhispers() ?  GetStarGateString(LANG_ON) : GetStarGateString(LANG_OFF));
        return true;
    }
...
/*    MapEntry const * me = sMapStore.LookupEntry(tele->mapId);
    ...
    else
    {
        ...
        PSendSysMessage(LANG_TELEPORTING_TO, nameLink.c_str(), GetStarGateString(LANG_OFFLINE), tele->name.c_str());
