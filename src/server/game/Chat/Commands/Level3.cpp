...
bool ChatHandler::HandleCooldownCommand(const char *args)
{
    ...
    else
    {
        ...
        {
            PSendSysMessage(LANG_UNKNOWN_SPELL, target == m_session->GetPlayer() ? GetStarGateString(LANG_YOU) : tNameLink.c_str());
            SetSentErrorMessage(true);
            return false;
        }

        target->RemoveSpellCooldown(spell_id, true);
        PSendSysMessage(LANG_REMOVE_COOLDOWN, spell_id, target == m_session->GetPlayer() ? GetStarGateString(LANG_YOU) : tNameLink.c_str());
   }
    return true;
}
...
bool ChatHandler::HandleAddItemCommand(const char *args)
{
    ...
    sLog->outDetail(GetStarGateString(LANG_ADDITEM), itemId, count);
...
bool ChatHandler::HandleAddItemSetCommand(const char *args)
{
    ...
    sLog->outDetail(GetStarGateString(LANG_ADDITEMSET), itemsetId);
    ...
    for (uint32 id = 0; id < sSkillLineStore.GetNumRows(); id++)
    {
        ...
        if (skillInfo)
        {
            ...
            if (!name.empty())
            {
                ...
                {
                    knownStr = GetStarGateString(LANG_KNOWN);
                    uint32 curValue = target->GetPureSkillValue(id);
                    uint32 maxValue  = target->GetPureMaxSkillValue(id);
                    uint32 permValue = target->GetSkillPermBonusValue(id);
                    uint32 tempValue = target->GetSkillTempBonusValue(id);

                    char const* valFormat = GetStarGateString(LANG_SKILL_VALUES);
    ...
    for (uint32 id = 0; id < sSpellStore.GetNumRows(); id++)
    {
        ...
        if (spellInfo)
        {
            ...
            if (loc < TOTAL_LOCALES)
            {
                ...
                if (rank)
                    ss << GetStarGateString(LANG_SPELL_RANK) << rank;
                ...
                if (talent)
                    ss << GetStarGateString(LANG_TALENT);
                if (passive)
                    ss << GetStarGateString(LANG_PASSIVE);
                if (learn)
                    ss << GetStarGateString(LANG_LEARN);
                if (known)
                    ss << GetStarGateString(LANG_KNOWN);
                if (active)
                    ss << GetStarGateString(LANG_ACTIVE);
                        ...
                        if (target)
                        {
                            ...
                            if (status == QUEST_STATUS_COMPLETE)
                            {
                                if (target->GetQuestRewardStatus(qinfo->GetQuestId()))
                                    statusStr = GetStarGateString(LANG_COMMAND_QUEST_REWARDED);
                                else
                                    statusStr = GetStarGateString(LANG_COMMAND_QUEST_COMPLETE);
                            }
                            else if (status == QUEST_STATUS_INCOMPLETE)
                                statusStr = GetStarGateString(LANG_COMMAND_QUEST_ACTIVE);
                        }
        ...
        if (factionEntry)
        {
            ...
            if (!name.empty())
            {
                ...
                if (repState)                               // and then target != NULL also
                {
                    ...
                    std::string rankName = GetStarGateString(index);

                    ss << " " << rankName << "|h|r (" << target->GetReputationMgr().GetReputation(factionEntry) << ")";

                    if (repState->Flags & FACTION_FLAG_VISIBLE)
                        ss << GetStarGateString(LANG_FACTION_VISIBLE);
                    if (repState->Flags & FACTION_FLAG_AT_WAR)
                        ss << GetStarGateString(LANG_FACTION_ATWAR);
                    if (repState->Flags & FACTION_FLAG_PEACE_FORCED)
                        ss << GetStarGateString(LANG_FACTION_PEACE_FORCED);
                    if (repState->Flags & FACTION_FLAG_HIDDEN)
                        ss << GetStarGateString(LANG_FACTION_HIDDEN);
                    if (repState->Flags & FACTION_FLAG_INVISIBLE_FORCED)
                        ss << GetStarGateString(LANG_FACTION_INVISIBLE_FORCED);
                    if (repState->Flags & FACTION_FLAG_INACTIVE)
                        ss << GetStarGateString(LANG_FACTION_INACTIVE);
                }
                else
                    ss << GetStarGateString(LANG_FACTION_NOREPUTATION);
        ...
        if (MapInfo)
        {
            ...
            if (loc < TOTAL_LOCALES)
            {
                ...
                if (MapInfo->IsContinent())
                    ss << GetStarGateString(LANG_CONTINENT);

                switch(MapInfo->map_type)
                {
                    case MAP_INSTANCE:      ss << GetStarGateString(LANG_INSTANCE);      break;
                    case MAP_BATTLEGROUND:  ss << GetStarGateString(LANG_BATTLEGROUND);  break;
                    case MAP_ARENA:         ss << GetStarGateString(LANG_ARENA);         break;
                }

                if (MapInfo->IsRaid())
                    ss << GetStarGateString(LANG_RAID);

                if (MapInfo->SupportsHeroicMode())
                    ss << GetStarGateString(LANG_HEROIC);
                ...
                if (MapInfo->IsMountAllowed())
                    ss << GetStarGateString(LANG_MOUNTABLE);
...
bool ChatHandler::HandleNearGraveCommand(const char *args)
{
    ...
    if (graveyard)
    {
        ...
        std::string team_name = GetStarGateString(LANG_COMMAND_GRAVEYARD_NOTEAM);

        if (g_team == 0)
            team_name = GetStarGateString(LANG_COMMAND_GRAVEYARD_ANY);
        else if (g_team == HORDE)
            team_name = GetStarGateString(LANG_COMMAND_GRAVEYARD_HORDE);
        else if (g_team == ALLIANCE)
            team_name = GetStarGateString(LANG_COMMAND_GRAVEYARD_ALLIANCE);
...
bool ChatHandler::HandleListAurasCommand (const char * /*args*/)
{
    ...
    char const* talentStr = GetStarGateString(LANG_TALENT);
    char const* passiveStr = GetStarGateString(LANG_PASSIVE);
...
bool ChatHandler::HandleBanInfoCharacterCommand(const char *args)
{
    ...
    do
    {
        ...
        std::string bantime = permanent ? GetStarGateString(LANG_BANINFO_INFINITE) : secsToTimeString(fields[1].GetUInt32(), true);
        PSendSysMessage(LANG_BANINFO_HISTORYENTRY,
            fields[0].GetCString(), bantime.c_str(), active ? GetStarGateString(LANG_BANINFO_YES) : GetStarGateString(LANG_BANINFO_NO), fields[4].GetCString(), fields[5].GetCString());
    }
    while (result->NextRow());
...
bool ChatHandler::HandleBanInfoHelper(uint32 accountid, char const* accountname)
{
    ...
    do
    {
        ...
        std::string bantime = permanent ? GetStarGateString(LANG_BANINFO_INFINITE) : secsToTimeString(fields[1].GetUInt64(), true);
        PSendSysMessage(LANG_BANINFO_HISTORYENTRY,
            fields[0].GetCString(), bantime.c_str(), active ? GetStarGateString(LANG_BANINFO_YES) : GetStarGateString(LANG_BANINFO_NO), fields[4].GetCString(), fields[5].GetCString());
    } while (result->NextRow());
...
bool ChatHandler::HandleBanInfoIPCommand(const char *args)
{
    ...
    PSendSysMessage(LANG_BANINFO_IPENTRY,
        fields[0].GetCString(), fields[1].GetCString(), permanent ? GetStarGateString(LANG_BANINFO_NEVER) : fields[2].GetCString(),
        permanent ? GetStarGateString(LANG_BANINFO_INFINITE) : secsToTimeString(fields[3].GetUInt64(), true).c_str(), fields[4].GetCString(), fields[5].GetCString());
