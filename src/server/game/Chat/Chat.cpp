...
const char *ChatHandler::GetStarGateString(int32 entry) const
{
    return m_session->GetStarGateString(entry);
}
...
{
    SendSysMessage(GetStarGateString(entry));
}

void ChatHandler::PSendSysMessage(int32 entry, ...)
{
    const char *format = GetStarGateString(entry);
    va_list ap;
    char str [2048];
    va_start(ap, entry);
...
const char *CliHandler::GetStarGateString(int32 entry) const
{
...
{
    return GetStarGateString(LANG_CONSOLE_COMMAND);
}
