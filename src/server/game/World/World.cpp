...
void World::LoadConfigSettings(bool reload)
{
    ...
    SetMotd(sConfig->GetStringDefault("Motd", "Welcome to a StarGate server."));
    ...
    m_int_configs[CONFIG_GM_LEVEL_ALLOW_ACHIEVEMENTS] = sConfig->GetIntDefault("GM.AllowAchievementGain.Level", 4);
    ...
    sLog->outString("Will löschen `logs` Einträge der Tabelle, die älter sind als %i Sekunden %u Minuten.",
    ...
    {
        m_dataPath = dataPath;
        sLog->outString("DataDir %s", m_dataPath.c_str());
    }
    ...
    sLog->outString("WORLD: VMap unterstützung enthalten. LineOfSight:%i, getHeight:%i", enableLOS, enableHeight);
    sLog->outString("WORLD: VMap Data-Verzeichnis: %svmaps", m_dataPath.c_str());
    sLog->outString("WORLD: VMap Config sind: vmap.enableLOS, vmap.enableHeight, vmap.ignoreMapIds, vmap.ignoreSpellIds");
    ...
    if (m_bool_configs[CONFIG_START_ALL_SPELLS])
        sLog->outString("WORLD: WARNUNG: PlayerStart.AllSpells aktiviert - möglicherweise nicht vorgesehen!");
...
void World::SetInitialWorldSettings()
{
    ...
    sLog->outString("Lade Autobroadcasts...");
    LoadAutobroadcasts();
    ...
    sLog->outString("Starte Map-System");
    sMapMgr->Initialize();

    sLog->outString("Starte Game-Event-System...");
    ...
    sLog->outString("Starte Arena-Season...");
    sGameEventMgr->StartArenaSeason();

    sTicketMgr->Initialize();

    sLog->outString("Lade World States...");              // must be loaded before battleground and outdoor PvP
    LoadWorldStates();

    ///- Initialize Battlegrounds
    sLog->outString("Starte Battleground-System");
    sBattlegroundMgr->CreateInitialBattlegrounds();
    sBattlegroundMgr->InitAutomaticArenaPointDistribution();

    ///- Initialize outdoor pvp
    sLog->outString("Starte Outdoor-PvP-System");
    sOutdoorPvPMgr->InitOutdoorPvP();

    sLog->outString("Lade Transports...");
    sMapMgr->LoadTransports();

    sLog->outString("Lade Transport NPCs...");
    sMapMgr->LoadTransportNPCs();

    sLog->outString("Lösche abgelaufene Banns");
    LoginDatabase.Execute("DELETE FROM ip_banned WHERE unbandate <= UNIX_TIMESTAMP() AND unbandate<>bandate");

    sLog->outString("Starte Objects-Pooling-System");
    sPoolMgr->Initialize();

    sLog->outString("[Berechnen Tägliche-Quest Reset-Zeit]");
    InitDailyQuestResetTime();

    sLog->outString("[Berechnen Wochen-Quest Reset-Zeit]" );
    InitWeeklyQuestResetTime();

    sLog->outString("[Berechnen Zufällige-Schlachtfelder Reset-Zeit]" );
    ...
    sLog->outString();
    sLog->outString("WORLD: World initialisiert in %u Minunten %u Sekunden", (startupDuration / 60000), ((startupDuration % 60000) / 1000) );
    sLog->outString();
}
...
void World::DetectDBCLang()
{
    ...
    {
        sLog->outError("Falsch DBC.Locale! muss >= 0 und < %d (set to 0)", TOTAL_LOCALES);
    ...
    {
        sLog->outError("Ihre DBC Locale ermitteln! (DBC?)");
        exit(1);
    }

    m_defaultDbcLocale = LocaleConstant(default_locale);

    sLog->outString("%s DBC Locale Standard. Alle verfügbaren DBC locales: %s", localeNames[m_defaultDbcLocale], availableLocalsStr.empty() ? "<none>" : availableLocalsStr.c_str());
    sLog->outString();
}
...
void World::LoadAutobroadcasts()
{
    ...
    if (!result)
    {
        sLog->outString(">> Lade 0 autobroadcasts Definitionen. DB table `autobroadcast` ist leer!");
        sLog->outString();
        return;
    }
    ...
    sLog->outString(">> Lade %u autobroadcasts Definitionen in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
}
...
void World::Update(uint32 diff)
{
    ...
    if (m_int_configs[CONFIG_INTERVAL_LOG_UPDATE] && diff > m_int_configs[CONFIG_MIN_LOG_UPDATE])
    {
        ...
        {
            sLog->outBasic("Update-Zeit diff: %u. Spieler Online: %u.", m_updateTimeSum / m_updateTimeCount, GetActiveSessionCount());
    ...
    if (m_timers[WUPDATE_PINGDB].Passed())
    {
        ...
        sLog->outDetail("Ping MySQL Verbindung aufrecht halten");
...
namespace Trinity
{
    class WorldWorldTextBuilder
    {
        public:
            ...
            {
                char const* text = sObjectMgr->GetStarGateString(i_textId, loc_idx);
...
void World::ShutdownMsg(bool show, Player* player)
{
        ...
        (m_ShutdownTimer > 12 * HOUR && (m_ShutdownTimer % (12 * HOUR)) == 0)) // > 12 h ; every 12 h
    {
        ...
        sLog->outStaticDebug("Server ist %s in %s", (m_ShutdownMask & SHUTDOWN_MASK_RESTART ? "restart" : "shuttingdown"), str.c_str());
    }
}
...
void World::ShutdownCancel()
{
    ...
    sLog->outStaticDebug("Server %s annulliert.", (m_ShutdownMask & SHUTDOWN_MASK_RESTART ? "restart" : "shuttingdown"));
...
{
    sLog->outDetail("Tägliche Quests für alle Charaktere zurückgesetzt.");
...
{
    sLog->outDetail("Zufällige BG-Status für alle Charaktere zurückgesetzt.");
...
{
    sLog->outDetail("Punkte-System für alle Charaktere zurückgesetzt.");
...
void World::LoadWorldStates()
{
    ...
    if (!result)
    {
        sLog->outString(">> Lade 0 world States. DB table `worldstates` ist leer!");
        sLog->outString();
        return;
    }
    ...
    sLog->outString(">> Lade %u world States in %u ms", count, GetMSTimeDiffToNow(oldMSTime));
    sLog->outString();
}
