            ...
            if (CanJoinConstantChannelInZone(channel, current_zone))
            {
                if (!(channel->flags & CHANNEL_DBC_FLAG_GLOBAL))
                {
                    ...
                    if (channel->flags & CHANNEL_DBC_FLAG_CITY_ONLY)
                        currentNameExt = sObjectMgr->GetStarGateStringForDBCLocale(LANG_CHANNEL_CITY);
                    else
...
void Player::SendQuestWindowClose(uint32 quest_id)
{
    if (quest_id)
    {
        WorldPacket data(SMSG_QUESTGIVER_QUEST_FAILED, 4+4);
...
void Player::_LoadInventory(PreparedQueryResult result, uint32 timediff)
{
    ...
    if (result)
    {
        ...
        {
            std::string subject = GetSession()->GetStarGateString(LANG_NOT_EQUIPPED_ITEM);
...
bool Player::Satisfy(AccessRequirement const* ar, uint32 target_map, bool report)
{
    if (!isGameMaster() && ar)
    {
        ...
        {
            GetSession()->SendAreaTriggerMessage("%s", GetSession()->GetStarGateString(LANG_INSTANCE_CLOSED));
            return false;
        }
        ...
        if (LevelMin || LevelMax || missingItem || missingQuest || missingAchievement)
        {
            if (report)
            {
                if (missingQuest && !ar->questFailedText.empty())
                    ...
                    GetSession()->SendAreaTriggerMessage(GetSession()->GetStarGateString(LANG_LEVEL_MINREQUIRED_AND_ITEM), LevelMin, ObjectMgr::GetItemPrototype(missingItem)->Name1);
                else if (LevelMin)
                    GetSession()->SendAreaTriggerMessage(GetSession()->GetStarGateString(LANG_LEVEL_MINREQUIRED), LevelMin);
            }
            return false;
        }
    }
